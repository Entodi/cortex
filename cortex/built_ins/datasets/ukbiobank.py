...
def zero_pad_center_crop(x, shape=[64, 64, 64], crop=[53, 63, 52]):
    coord1 = 5
    coord2 = 0
    coord3 = 6
    ...
    return torch.from_numpy(ret[:, :shape[0], :shape[1], :shape[2]])
...
class UKBiobankPlugin(DatasetPlugin):
    sources = ['ukbiobank0', 'ukbiobank1', 'ukbiobank2', 'ukbiobank3', 'ukbiobank4']
    ...
    def handle(self, source, copy_to_local=False, **transform_args):
        ...
        train_set.classes = pd.read_csv(train_path + 'targets.csv').values
        test_set.classes = pd.read_csv(test_path + 'targets.csv').values

        dim_c, dim_x, dim_y, dim_z = train_set[0][0].size()
        print (test_set.classes.min(), test_set.classes.max())
        dim_l = 1
...
register_plugin(UKBiobankPlugin)
