...
def finish_layer_3d(models, name, dim_x, dim_y, dim_z, dim_out,
    ...
    if dropout:
        models.add_module(name + '_do', nn.Dropout3d(p=dropout))

    if layer_norm:
        models.add_module(name + '_ln', nn.LayerNorm((dim_out, dim_x, dim_y, dim_z)))
    elif batch_norm:
        models.add_module(name + '_bn', nn.BatchNorm3d(dim_out))
